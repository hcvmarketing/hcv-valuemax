...
function resultRow(label, value, highlight){
  return `<div class="summary-stat ${highlight?'hl':''}"><span>${label}</span><span class="v">${value}</span></div>`;
}
...
function renderFe(){
  ...
  const yrsInt=Math.max(1,Math.round(i.years));
  ...
  html += `<div class="card" id="feStep2"><div class="sect-title">STEP 2 &mdash; Current monthly fuel cost</div>
    ${resultRow('Diesel consumed / month', num(base.litresPerMonth,0)+' L')}
    ${resultRow('Monthly fuel cost', inr(base.costPerMonth))}
    ${resultRow('Yearly fuel cost', inr(base.costPerYear), true)}
  </div>`;

  const rangeNote = (s.selectedPct>=7 && s.selectedPct<=10) ? '  (FE Series range)' : '';
  html += `<div class="card"><div class="sect-title">STEP 3 &mdash; Savings with the new FE Series truck</div>
    <div class="foot-note" style="margin:0 0 8px">FE Series delivers 7&ndash;10% better fuel efficiency. Slide to show the customer:</div>
    <input type="range" min="1" max="12" value="${s.selectedPct}" oninput="feSlideLive(this.value)" onchange="feSlide(this.value)">
    <div class="slider-val" id="feSliderLabel">Mileage improvement: ${s.selectedPct}%${rangeNote}</div>
    <div id="feHero">${sel?feHeroBlock(sel,yrsInt):''}</div>
  </div>`;

  // in-app savings table (kept in app, per requirements)
  if(table.length){
    html += `<div class="card"><div class="sect-title">Full table (1% &ndash; 12%)</div>
      <table class="cmp" id="feTable"><tr><th>FE&uarr;</th><th>km/l</th><th>&#8377;/month</th><th>&#8377;/year</th><th>&#8377;/${yrsInt} yrs</th></tr>
      ${table.map(r=>`<tr data-pct="${r.pct}" class="${r.pct===s.selectedPct?'bold':''}"><td>${r.pct}%</td><td>${num(r.newMileage,2)}</td><td>${num(r.savePerMonth,0)}</td><td>${num(r.savePerYear,0)}</td><td>${num(r.saveOverYears,0)}</td></tr>`).join('')}
      </table></div>`;
  }
  ...
  html += `<div class="foot-note">Note: Actual fuel efficiency depends on road conditions, load, driver habits and maintenance. Indicative planning tool only.</div>`;
  document.getElementById('screen-fe').innerHTML=html;
}
function feHeroBlock(sel, yrsInt){
  return heroBox('Savings per year at '+sel.pct+'% better FE', sel.savePerYear,
      num(sel.litresSaved,0)+' litres saved every month  &bull;  '+yrsInt+'-year savings: '+inrWords(sel.saveOverYears))
    + resultRow('New mileage', num(sel.newMileage,2)+' km/l')
    + resultRow('Savings / month', inr(sel.savePerMonth))
    + resultRow('Savings in '+yrsInt+' years', inr(sel.saveOverYears), true);
}
function softUpdateFe(){ // update fuel-cost step + hero without full re-render (keeps keyboard focus)
  const s=State.fe; saveDraft('fe');
  const i={monthlyKm:toD(s.monthlyKm),mileage:toD(s.mileage),diesel:toD(s.diesel),years:toD(s.years,5)};
  const base=FeLogic.baseline(i);
  const yrsInt=Math.max(1,Math.round(i.years));
  const sel=FeLogic.savingsTable(i).find(r=>r.pct===s.selectedPct);
  const step2=document.getElementById('feStep2');
  if(step2) step2.innerHTML=`<div class="sect-title">STEP 2 &mdash; Current monthly fuel cost</div>
    ${resultRow('Diesel consumed / month', num(base.litresPerMonth,0)+' L')}
    ${resultRow('Monthly fuel cost', inr(base.costPerMonth))}
    ${resultRow('Yearly fuel cost', inr(base.costPerYear), true)}`;
  const h=document.getElementById('feHero');
  if(h&&sel) h.innerHTML=feHeroBlock(sel,yrsInt);
}
function softUpdateFeMileage(){ // update error state + step2 + hero, without rebuilding the input (avoids focus loss / scroll jump)
  const s=State.fe;
  const mileageErr = s.mileage!=='' && toD(s.mileage)<=0 ? 'Must be greater than 0' : '';
...
function feSlideLive(v){
  ...
  const rangeNote=(s.selectedPct>=7 && s.selectedPct<=10) ? '  (FE Series range)' : '';
  if(lab) lab.textContent='Mileage improvement: '+s.selectedPct+'%'+rangeNote;
  const h=document.getElementById('feHero');
  if(h&&sel) h.innerHTML=feHeroBlock(sel,yrsInt);
...
function renderHpt(){
  ...
  html+=`<div class="card"><div class="sect-title">Vehicle selection</div>
    ...
    ${numField('Additional payload', s.addlPayload, v=>{s.addlPayload=v;}, {suffix:'tonnes', hint:'Auto-filled from tonnage point ('+pt.label+' vs '+pt.base+'); editable if needed', rerender:()=>softUpdateHpt()})}
  </div>`;
  html+=`<div class="card"><div class="sect-title">Operating inputs</div>
    ${numField('Loaded running', s.loadedPct, v=>{s.loadedPct=v;}, {suffix:'%', err:!!loadedErr, hint:(loadedErr||'Share of running done with full load'), rerender:()=>softUpdateHpt()})}
  ...
  html+=`<div id="hptHero">
    ${heroBox('Annual revenue advantage', res.annual, 'Extra freight earned from +'+toD(s.addlPayload)+'T payload')}
  ...
  html+=`<div class="foot-note">Every extra tonne carried is direct revenue &mdash; same trip, same fuel, more earnings.</div>`;
...
function softUpdateHpt(){
  ...
  if(h) h.innerHTML=heroBox('Annual revenue advantage', res.annual, 'Extra freight earned from +'+toD(s.addlPayload)+'T payload')+
    heroBox('Lifetime revenue advantage ('+yrs+' years)', res.lifetime);
}
...
function productForm(p, idx){
  ...
  // tyres
  let tyreInner='<div class="foot-note" style="margin-bottom:8px">Add one row per axle group. Tyre cost/km = &Sigma; (count &times; cost &divide; life). Works for any configuration &mdash; rigid trucks, tractor-trailers, multi-axle.</div>';
  ...
  html+=collapsible('Other running &amp; fixed costs',
    ...
    numField('Insurance & taxes', p.insurancePct, v=>{p.insurancePct=v;}, {suffix:'% of vehicle+body cost', hint:'BRT default: 4%',rerender:softBrt}), false);
  return html;
}
...
function brtResults(s,c,tmlWins,winner){
  ...
  html+=`<div class="foot-note">Insurance taken as % of initial cost (BRT default 4%). EMI uses standard reducing-balance PMT over (tenure &minus; moratorium) months. Indicative planning tool.</div>`;
  return html;
}
...
function buildBrtPdf(s){
  ...
  p.tagline('Ab Profit Hoga Aur Bhi Zyaada','Business Return Template  |  A practical approach to make transport business decisions');
