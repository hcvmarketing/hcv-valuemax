...
function field(label, val, oninput, opts){
  ...
  const hint=opts.hint?`<div class="hint ${opts.err?'err':''}">${opts.hint}</div>`:'';
  const errcls=opts.err?'err':'';
  const type=opts.text?'text':'text';
  const inputmode=opts.text?'':'inputmode="decimal"';
  window['_h_'+id]=oninput;
  return `<div class="field ${errcls}"><label>${label}</label><div class="wrap">
...
function numField(label, rawVal, setter, opts){
  ...
  return field(label, disp, function(el){
    ...
    if(raw==='' || /^\d*\.?\d*$/.test(raw)){ setter(raw); if(opts.grouped){ const p=el.selectionStart; el.value=groupIndian(raw); } if(opts.rerender) opts.rerender(); }
    else { el.value=disp; }
  }, opts);
}
...
function renderFe(){
  ...
  html += `<div class="card"><div class="sect-title">Step 1 &mdash; Vehicle &amp; usage</div>
    ...
    ${numField('Current mileage', s.mileage, v=>{s.mileage=v;}, {suffix:'km/l', err:!!mileageErr, hint:mileageErr, rerender:()=>renderFe()})}
    <div class="row2">
  ...
  html += `<div class="card"><div class="sect-title">Step 2 &mdash; Savings with FE Series</div>
    <div class="slider-val">Mileage improvement: ${s.selectedPct}% &nbsp;<span style="color:var(--grey);font-weight:400">(FE Series range 7&ndash;10%)</span></div>
    <input type="range" min="1" max="12" value="${s.selectedPct}" oninput="feSlide(this.value)">
  ...
  if(table.length){
    html += `<div class="card"><div class="sect-title">Full savings table (1%&ndash;12%)</div>
      <table class="cmp"><tr><th>FE%</th><th>km/l</th><th>Saved L/mo</th><th>&#8377;/month</th><th>&#8377;/year</th></tr>
      ${table.map(r=>`<tr class="${r.pct===s.selectedPct?'bold':''}"><td>${r.pct}%</td><td>${num(r.newMileage,2)}</td><td>${num(r.litresSaved)}</td><td>${inr(r.savePerMonth)}</td><td>${inr(r.savePerYear)}</td></tr>`).join('')}
      </table></div>`;
  }
...
function renderHpt(){
  ...
  html+=`<div class="card"><div class="sect-title">Operating inputs</div>
    ...
    ${numField('Monthly running', s.monthlyKm, v=>{s.monthlyKm=v;}, {suffix:'km', grouped:true, hint:'Yearly running = '+num(toD(s.monthlyKm)*12)+' km', rerender:()=>renderHpt()})}
...
function brtResults(s,c,tmlWins,winner){
  ...
  html+=heroBox(tmlWins?('Extra profit per year with '+n1):('Profit gap per year vs '+n2), c.extraProfitPerYear,
    'Over '+yrs+' years of operation: '+inrWords(c.extraProfitOverYears));
  html+=`<div class="card">${numField('Years of operation', s.compareYears, v=>{s.compareYears=v;}, {suffix:'years', rerender:()=>renderBrt()})}</div>`;
...
function PdfDoc(){
  ...
  function header(){
    // lockup top-right
    if(ASSETS.lockup){ try{ doc.addImage(ASSETS.lockup,'PNG',PW-160,24,120,19); }catch(e){} }
    doc.setDrawColor(220).setLineWidth(0.5).line(M,52,PW-M,52);
    y=64;
  }
  function footer(){
    doc.setFillColor(...PDFC.dark).rect(0,PH-56,PW,56,'F');
    doc.setTextColor(74,174,233).setFont('helvetica','bold').setFontSize(11).text('TATA TRUCKS',M,PH-34);
    doc.setTextColor(255).setFontSize(13).text('DESH KE TRUCKS',M,PH-18);
    doc.setTextColor(180,186,193).setFont('helvetica','normal').setFontSize(7)
      .text('Generated by HCV ValueMaX  •  Indicative planning tool',PW-M,PH-18,{align:'right'});
  }
  function ensure(h){ if(y+h>PH-70){ footer(); doc.addPage(); header(); } }
  header();
  return {
    doc,PW,PH,M,
    tagline(t,s){ ensure(52); doc.setFillColor(...PDFC.blue).roundedRect(M,y,PW-2*M,44,5,5,'F');
      doc.setTextColor(255).setFont('helvetica','bold').setFontSize(15).text(t,M+12,y+20);
      doc.setFont('helvetica','normal').setFontSize(8).setTextColor(211,230,248).text(s,M+12,y+34); y+=54; },
    banner(img,maxH){ if(!img) return; try{ const props=doc.getImageProperties(img); const w=PW-2*M; let h=w*props.height/props.width; if(h>maxH)h=maxH;
      ensure(h+8); doc.addImage(img,'JPEG',M,y,w,h); y+=h+10; }catch(e){} },
    section(t){ ensure(24); doc.setTextColor(...PDFC.blueDark).setFont('helvetica','bold').setFontSize(10).text(t.toUpperCase(),M,y+10); y+=22; },
    kv(pairs){ doc.setFontSize(9); pairs.forEach(([k,v])=>{ ensure(14);
      doc.setFont('helvetica','normal').setTextColor(...PDFC.grey).text(k,M,y+9);
      doc.setFont('helvetica','bold').setTextColor(...PDFC.dark).text(String(v),M+165,y+9); y+=14; }); y+=6; },
    hero(cap,amount,sub){ ensure(76); doc.setFillColor(...PDFC.greenLt).roundedRect(M,y,PW-2*M,66,10,10,'F');
      doc.setTextColor(...PDFC.grey).setFont('helvetica','bold').setFontSize(10).text(cap,M+14,y+20);
      doc.setTextColor(...PDFC.green).setFontSize(22).text(inrWords(amount)+' / year',M+14,y+44);
      if(sub){ doc.setTextColor(...PDFC.dark).setFont('helvetica','normal').setFontSize(9).text(sub,M+14,y+59); } y+=76; },
    ...
    table(headers,rows,weights,opts){ opts=opts||{}; const tableW=PW-2*M,rowH=18; const xs=[]; let acc=M;
      weights.forEach(w=>{xs.push(acc);acc+=tableW*w;}); ensure(rowH*2);
      doc.setFillColor(...PDFC.blue).rect(M,y,tableW,rowH,'F');
      doc.setTextColor(255).setFont('helvetica','bold').setFontSize(8.5);
      headers.forEach((h,i)=>doc.text(String(h),xs[i]+5,y+12,i>0&&opts.rightCols?{align:'left'}:{})); y+=rowH;
      rows.forEach((row,r)=>{ ensure(rowH); if(r%2===1){ doc.setFillColor(...PDFC.rowAlt).rect(M,y,tableW,rowH,'F'); }
        ...
        row.forEach((cell,i)=>{ doc.setFont('helvetica',(bold||green===i)?'bold':'normal').setFontSize(8.5)
          .setTextColor(...(green===i?PDFC.green:PDFC.dark)).text(String(cell),xs[i]+5,y+12); }); y+=rowH; }); y+=10; },
...
function buildFePdf(s){
  ...
  p.tagline('Go Further with Every Drop','FE Series Trucks  |  7-10% Better Fuel-Efficiency');
  p.banner(ASSETS.fe,80);
  ...
  p.kv(customerPairs(s,[['Vehicle / Tonnage',s.vehicle],['Monthly running',num(i.monthlyKm)+' km'],
    ...
    ['FE improvement applied',sel.pct+' %  (FE Series: 7-10%)'],['Years of operation',yrs+' years']]));
  ...
  p.hero('Savings at '+sel.pct+'% better fuel efficiency',sel.savePerYear,'New mileage: '+num(sel.newMileage,2)+' km/l');
  p.quickView('Quick view - savings at a glance',[
    ['Diesel saved / month',num(sel.litresSaved)+' litres',false],['Savings / month',inr(sel.savePerMonth),false],
    ['Savings / year',inr(sel.savePerYear),true],['Savings over '+yrs+' years',inr(sel.saveOverYears),true]]);
  p.note('FE Series delivers 7-10% better fuel efficiency vs MY2025 and earlier models. T&C apply. Actual fuel efficiency depends on road conditions, load, driver habits and maintenance. Indicative planning tool only.');
...
function buildHptPdf(s){
  ...
  p.tagline('Carry More. Earn More','Higher Payload Trucks  |  Designed for versatile loads');
  p.banner(ASSETS.hpt,80);
  p.section('Customer details & inputs');
  p.kv(customerPairs(s,[['Tonnage point',pt.label+' (vs base '+pt.base+')'],['Additional payload',toD(s.addlPayload)+' tonnes'],
    ['Loaded running',toD(s.loadedPct)+' %'],['Monthly running',num(toD(s.monthlyKm))+' km'],['Yearly running',num(res.yearlyKm)+' km'],
    ['Freight rate','Rs '+toD(s.rate)+' per tonne-km'],['Years of operation',yrs+' years']]));
  p.hero('Annual revenue advantage',res.annual,'From carrying '+toD(s.addlPayload)+' tonnes more on the same trip');
  p.quickView('Quick view - revenue advantage at a glance',[
    ['Tonnage point',pt.label+' (vs base '+pt.base+')',false],['Additional payload',toD(s.addlPayload)+' tonnes',false],
    ['Revenue advantage / year',inrWords(res.annual),true],['Lifetime advantage ('+yrs+' years)',inrWords(res.lifetime),true]]);
  p.note('Every extra tonne carried is direct revenue - same trip, same route, more earnings. Indicative planning tool; actual earnings depend on load availability, freight rates and route conditions.');
...
function buildBrtPdf(s){
  ...
  p.banner(ASSETS.brt,80);
  p.section('Customer details');
  p.kv(customerPairs(s));
  p.hero('Extra operating profit per year with '+winner,Math.abs(c.extraProfitPerYear),'Over '+yrs+' years of operation: '+inrWords(Math.abs(c.extraProfitOverYears)));
  p.quickView('Quick view - business return at a glance',[
    [n1+' - profit / year',inrWords(r1.operatingProfitPerYear),true],
    [n2+' - profit / year',inrWords(r2.operatingProfitPerYear),false],
    ['Extra profit / year with '+winner,inrWords(Math.abs(c.extraProfitPerYear)),true],
    ['Extra profit over '+yrs+' years',inrWords(Math.abs(c.extraProfitOverYears)),true],
    ['Cost per km',inr(r1.costPerKm,2)+' vs '+inr(r2.costPerKm,2),false]]);
  ...
  p.section('Key assumptions used');
  p.table(['Assumption',n1,n2],[
    ['Vehicle + body price',inr(r1.initialCost),inr(r2.initialCost)],
    ['Mileage (km/l)',num(toD(s.product1.mileage),2),num(toD(s.product2.mileage),2)],
    ['Fuel price (Rs/l)',num(toD(s.product1.fuelPrice),2),num(toD(s.product2.fuelPrice),2)],
    ['Funding / interest',toD(s.product1.fundingPct)+'% / '+toD(s.product1.interestPct)+'%',toD(s.product2.fundingPct)+'% / '+toD(s.product2.interestPct)+'%'],
    ['Tenure (years)',num(toD(s.product1.tenureYears)),num(toD(s.product2.tenureYears))],
    ['Tyre cost / km',inr(BrtLogic.tyrePerKm(brtModel(s.product1).tyreGroups),2),inr(BrtLogic.tyrePerKm(brtModel(s.product2).tyreGroups),2)]],
    [0.46,0.27,0.27]);
  ...
  p.note('EMI: reducing-balance over (tenure - moratorium) months. Insurance taken as % of initial cost. Indicative planning tool; actual results vary with road conditions, load, driver habits and maintenance.');
